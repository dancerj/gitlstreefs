...
int PtfsHandler::Utimens(const std::string& relative_path, const struct timespec ts[2]) {
  WRAP_ERRNO(utimensat(premount_dirfd_,
		       relative_path.c_str(), ts, AT_SYMLINK_NOFOLLOW));
}

int PtfsHandler::Mknod(const std::string& relative_path, mode_t mode, dev_t rdev) {
  WRAP_ERRNO(mknodat(premount_dirfd_, relative_path.c_str(), mode, rdev));
}

int PtfsHandler::Link(const std::string& relative_path_from, const std::string& relative_path_to) {
...
int PtfsHandler::Readlink(const string& relative_path, char* buf, size_t size) {
  int res;
  if ((res = readlinkat(premount_dirfd_,
...
int PtfsHandler::Rename(const string& relative_path_from, const string& relative_path_to) {

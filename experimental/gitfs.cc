...
100644 blob f32d521227f2912fdc570418405c7360e42e9c93      49
statistics/README
...
GitTree::GitTree(const char* revision_ref, const string& gitdir)
  ...
  root_.reset(new FileElement(S_IFDIR, GIT_OBJ_TREE, "TODO", 0, nullptr));
...
void GitTree::LoadDirectory(FileElement::FileElementMap* files,
                            const string& subdir, gitxx::Tree* tree) {
  tree->for_each_file(
      [&](const string& name, int attribute, git_otype file_type,
          const string& sha1, int size, unique_ptr<gitxx::Object> object) {
        FileElement* fe =
            new FileElement(attribute, file_type, sha1, size, move(object));
        (*files)[name].reset(fe);
...
static void dumpFiles(const FileElement::FileElementMap* files) {
  for (const auto& it : *files) {
    ...
    cout << it.first << ":\t" << f->attribute_ << " "
         << file_type_to_string_map[(int)f->file_type_] << " " << f->sha1_
...
void GitTree::dump() const { dumpFiles(&root_->files_); }

...
struct FileElement {
 public:
  FileElement(int attribute, git_otype file_type, const std::string& sha1,
              int size, std::unique_ptr<gitxx::Object> object);
  int attribute_;
  ...
  void for_each_filename(
      std::function<void(const std::string& filename)> callback) const {
  ...
  typedef std::unordered_map<std::string, std::unique_ptr<FileElement> >
      FileElementMap;
...
class GitTree {
 ...
 private:
  ...
  void LoadDirectory(FileElement::FileElementMap* files,
                     const std::string& subdir, gitxx::Tree* tree);
  // Full path without starting /
  std::unordered_map<std::string, FileElement*> fullpath_to_files_;

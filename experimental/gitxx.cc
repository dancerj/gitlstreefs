...
GitSingleton::GitSingleton() { git_libgit2_init(); }
GitSingleton::~GitSingleton() { git_libgit2_shutdown(); }
...
void Tree::for_each_file(
    function<void(const string& name, int attribute, git_otype file_type,
                  const string& sha1, int size, unique_ptr<Object>)>
        file_handler,
  ...
  for (i = 0; i < max_i; ++i) {
    ...
    file_handler(name, git_tree_entry_filemode(te), git_tree_entry_type(te),
                 oidstr, size, move(cxx_object));
...
void Tree::dump(const string& path_prepend) const {
  for_each_file(
      [&](const string& name, int attribute, git_otype file_type,
          const string& sha1, int size, unique_ptr<Object> object) {
        // File
        printf("%06o %s %s\t%s%s\n", attribute,
               git_object_type2string(file_type), sha1.c_str(),
               path_prepend.c_str(), name.c_str());
      },
...
Object::Object(const Repository* repo, const string& rev) : repo_(repo) {
  git_revparse_single(&obj_, repo_->repo, rev.c_str());
}
Object::Object(const Repository* repo, git_object* object)
    : obj_(object), repo_(repo) {}
